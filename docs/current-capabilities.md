...
| 酒店 | `hotel.search` | `帮我搜索2026年8月8日至10日深圳南山区科技园附近的酒店，2位成人1间房` | RollingGo 真实酒店结果、地址、参考价与可展开详情；不伪造库存 | `read` | `ROLLINGGO_HOTEL_MCP_KEY` / `ROLLINGGO_HOTEL_MCP_URL` | 取决于 RollingGo 网络 | 否 | core C20 |
| 酒店 | `hotel.detail` | 从 C20 的真实酒店卡点击“查看实时房型” | 使用上一步真实 hotelId 查询房型、价格和取消政策；不预订 | `read` | 同 `hotel.search`，且必须保留真实 hotelId | 取决于 RollingGo 网络 | 否 | core C20 衍生交互 |
