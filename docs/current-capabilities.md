...
| 酒店 | `hotel.navigate` | 从 C20 的真实酒店卡点击“导航到酒店” | 仅使用该酒店返回的有效坐标打开系统地图；不创建预订 | `read` | 无 | 取决于系统地图 | 否 | core C20 衍生交互 |
| 酒店 | `hotel.call` | 从 C20 的真实酒店卡点击“联系酒店” | 仅在 Google Places 验证电话和 place ID 后打开系统拨号盘；不自动拨打、不创建预订 | `draft` | Google Places key 和已验证联系人 | 取决于 Google Places 与系统拨号 | 否 | core C20 衍生交互 |
