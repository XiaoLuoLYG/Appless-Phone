...
`ToolDefinitionRegistry` 当前有 44 个固定工具。目标架构仍以它为唯一能力权威，不复制每个 Agent 自己的工具表。
...
目标 registry 中固定工具仍为 44 个，即 24 个 Data 工具加 20 个 Action 工具；`blocked` 固定工具目标为 0。这里的“Action 工具”包含 draft、preview、confirm-required、system intent 和 web session，不表示都可无确认执行。
...
- 44 个固定 ToolDefinition 恰好一个 owner；
- 24 Data + 20 Action；
