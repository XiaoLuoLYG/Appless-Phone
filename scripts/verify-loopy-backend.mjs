...
function matchingBraceEnd(masked, openIndex) {
  let depth = 0;
...
function liveDeclarationRange(source, declarationPattern) {
  const masked = maskCommentsAndStrings(source);
  const match = declarationPattern.exec(masked);
  if (match === null) {
    return null;
  }
  const openIndex = masked.indexOf('{', match.index);
  const closeIndex = openIndex < 0 ? -1 : matchingBraceEnd(masked, openIndex);
  if (closeIndex < 0) {
    return null;
  }
  return {
    source: source.substring(openIndex + 1, closeIndex),
    masked: masked.substring(openIndex + 1, closeIndex)
  };
}

function hasLiveGmailPromptCallsite(source, declarationPattern) {
  const range = liveDeclarationRange(source, declarationPattern);
  if (range === null) {
    return false;
  }
  const branchPattern = /(?:else\s+)?if\s*\(\s*definition\.toolId\s*===\s*(['"])gmail\.mail\.search\1\s*\)\s*\{/g;
  for (const match of range.source.matchAll(branchPattern)) {
    const ifOffset = match[0].search(/\bif\b/);
    const ifIndex = (match.index ?? 0) + ifOffset;
    if (ifOffset < 0 || range.masked.substring(ifIndex, ifIndex + 2) !== 'if') {
      continue;
    }
    const openIndex = range.masked.indexOf('{', (match.index ?? 0) + match[0].length - 1);
    const closeIndex = openIndex < 0 ? -1 : matchingBraceEnd(range.masked, openIndex);
    if (closeIndex > openIndex &&
      /\bGMAIL_SEARCH_MODEL_INSTRUCTIONS\b/.test(range.masked.substring(openIndex + 1, closeIndex))) {
      return true;
    }
  }
  return false;
...
function hasExactGmailSearchPromptContract(source) {
  const masked = maskCommentsAndStrings(source);
  const declaration = /\bconst\s+GMAIL_SEARCH_MODEL_INSTRUCTIONS\s*:\s*string\s*=/.exec(masked);
  if (declaration === null) {
    return false;
  }
  const endIndex = masked.indexOf(';', declaration.index + declaration[0].length);
  if (endIndex < 0) {
    return false;
  }
  const initializer = source.substring(declaration.index, endIndex + 1);
  if (!initializer.includes('only from exact keywords explicitly present') ||
    !initializer.includes('Never add synonyms') ||
    initializer.includes('expand the Gmail query with likely English terms')) {
    return false;
  }
  return hasLiveGmailPromptCallsite(
    source,
    /\bfunction\s+calendarToolDescription\s*\([^)]*\)\s*:\s*string\s*\{/
  ) && hasLiveGmailPromptCallsite(
    source,
    /\bprivate\s+describeAiphoneTool\s*\([^)]*\)\s*:\s*string\s*\{/
  );
}

function verifyArchitectureVerifier() {
  const gmailPromptFixture = `
    const GMAIL_SEARCH_MODEL_INSTRUCTIONS: string =
      'Build args.query only from exact keywords explicitly present in the current user request. ' +
      'Never add synonyms.';
    function calendarToolDescription(definition: ToolDefinition): string {
      if (definition.toolId === 'gmail.mail.search') {
        return description + GMAIL_SEARCH_MODEL_INSTRUCTIONS;
      }
      return description;
    }
    class LoopBackend {
  ...
  `;
  assert(
    hasExactGmailSearchPromptContract(gmailPromptFixture),
    'verifier accepts live exact Gmail prompt declaration and both callsites'
  );
  assert(
    !hasExactGmailSearchPromptContract(`
      // const GMAIL_SEARCH_MODEL_INSTRUCTIONS: string =
      //   'Build args.query only from exact keywords explicitly present in the current user request. Never add synonyms.';
      function calendarToolDescription(definition: ToolDefinition): string {
        // if (definition.toolId === 'gmail.mail.search') return GMAIL_SEARCH_MODEL_INSTRUCTIONS;
        return description;
      }
      class LoopBackend {
        private describeAiphoneTool(definition: ToolDefinition): string {
    ...
    `),
    'verifier rejects comment-only Gmail prompt decoys'
  );
  assert(
    !hasExactGmailSearchPromptContract(`
      const decoy: string = \`const GMAIL_SEARCH_MODEL_INSTRUCTIONS: string =
        'Build args.query only from exact keywords explicitly present in the current user request. Never add synonyms.';\`;
      function calendarToolDescription(definition: ToolDefinition): string {
        return 'GMAIL_SEARCH_MODEL_INSTRUCTIONS';
      }
      class LoopBackend {
        private describeAiphoneTool(definition: ToolDefinition): string {
          return "GMAIL_SEARCH_MODEL_INSTRUCTIONS";
    ...
    `),
    'verifier rejects string-only Gmail prompt decoys'
  );
  assert(
    !hasExactGmailSearchPromptContract(gmailPromptFixture.replace(
      'description += GMAIL_SEARCH_MODEL_INSTRUCTIONS;',
      "description += 'GMAIL_SEARCH_MODEL_INSTRUCTIONS';"
    )),
    'verifier rejects a missing live Gmail prompt callsite'
  );
...
function verifySourceContracts() {
  ...
  assert(
    hasExactGmailSearchPromptContract(backend),
    'LoopBackend binds exact Gmail keyword guidance to both live tool description paths'
  );
